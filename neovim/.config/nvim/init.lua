  ...
  -- Formatter: Conform (Go uses gofumpt + goimports)
  {
    ...
    opts = {
      ...
      format_on_save = function(bufnr)
        local disable_ft = {}
        if disable_ft[vim.bo[bufnr].filetype] then return end
...
keymap("n", "<leader>td", function() require("neotest").run.run({ strategy = "dap" }) end, { desc = "Debug nearest test" })
...
keymap("n", "<leader>do", function() require("dap").step_over() end, { desc = "DAP: Step Over" })
keymap("n", "<leader>di", function() require("dap").step_into() end, { desc = "DAP: Step Into" })
keymap("n", "<leader>dr", function() require("dap").repl.toggle() end, { desc = "DAP: Toggle REPL" })
...
vim.api.nvim_create_autocmd("LspAttach", {
  callback = function(ev)
    ...
    if vim.lsp.inlay_hint then
      local ok1, ih = pcall(function() return vim.lsp.inlay_hint end)
      if ok1 then
